...
import { Role } from './Role'; // Import the Role file
import './Profile.css'; 
...
const UserInfo = () => {
  ...
  const storedUser = JSON.parse(localStorage.getItem('user'));
  const isAdmin = storedUser?.role === Role.Admin;

  return (
    <Card className="user-info-card mb-2">
      <Card.Body>
        ...
        {isEditing ? (
          ...
          <>
            <div className="user-info-content mt-5">
              <div className="user-info-column">
                ...
                {!isAdmin && <p><strong>Grade:</strong> {gradeName}</p>}
                <p><strong>Email:</strong> {user.Email || 'N/A'}</p>
                <p><strong>Phone:</strong> {user.Phone || 'N/A'}</p>
                {!isAdmin && <p><strong>Created Date:</strong> {user.CreateDate || 'N/A'}</p>}
              </div>
            </div>
