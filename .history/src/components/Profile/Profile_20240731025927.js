  ...
  const handleSubmit = async (e) => {
    ...
    if (userId) {
      try {
        ...
        // Re-fetch the user data to ensure it reflects the latest changes
        const response = await axios.get(`http://localhost:8000/api/user/${userId}`);
        setUser(response.data);
        setIsEditing(false);
        setError(null);
      ...
      <Card.Body>
        {isEditing ? (
          ...
          <>
            ...
            <p><strong>Grade:</strong> {grades.find(grade => grade.id === user.examinee?.GradeId)?.name || 'N/A'}</p>
            <p><strong>Email:</strong> {user.Email || 'N/A'}</p>
            <p><strong>Phone:</strong> {user.Phone || 'N/A'}</p>
            <p><strong>Created Date:</strong> {user.CreateDate || 'N/A'}</p>
            <div className="text-center mt-3">
              <Button variant="primary" onClick={handleEdit}>Update your information</Button>
            </div>
          </>
        )}
      </Card.Body>
