...
const UserInfo = () => {
  ...
  const fetchUserAndGrade = useCallback(async () => {
    ...
    if (userId) {
      try {
        ...
        const userGrade = gradeResponse.data.find(g => g.id === userResponse.data.examinee.GradeId);
        setGrade(userGrade || { name: 'Unknown' });
        setError(null);
      } catch (err) {
  ...
  const formatGender = (gender) => {
    return gender === 'M' ? 'Male' : gender === 'F' ? 'Female' : gender === 'O' ? 'Other' : 'Unknown';
  };
