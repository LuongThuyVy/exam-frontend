...
const ExamShiftManager = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (newShift.Name === '' || newShift.StartTime === '' || newShift.EndTime === '' || newShift.ExamId === '') {
    ...
    try {
      ...
      setSuccess('Shift saved successfully.');
      handleCloseModal();
    } catch (error) {
      console.error('Error handling shift:', error);
      setError('An error occurred while saving the exam shift.');
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setError('An error occurred while deleting the exam shift.');
    }
  };
