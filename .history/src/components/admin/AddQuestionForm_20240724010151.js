...
function QuestionForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
        const subjectGradeId = formData.SubjectGradeId; // Đảm bảo rằng đây là số nguyên
        await axios.post('http://localhost:8000/api/questions', {
            ...formData,
            SubjectGradeId: parseInt(subjectGradeId, 10) // Chuyển đổi thành số nguyên
        });
        // Handle success (e.g., show a success message or redirect)
    } catch (err) {
        console.error('Error adding question:', err.response.data.errors);
