...
const ExamManagement = () => {
  ...
  const handleAddQuestion = (examId) => {
    history.push(`/add-question/${examId}`); // Navigate to the add question page with the exam ID
  };
          ...
          {exams.map(exam => (
            <tr key={exam.id}>
              ...
              <td>
                ...
                <Button variant="success" onClick={() => handleAddQuestion(exam.id)}>Add Question</Button> {/* Add this button */}
              </td>
            </tr>
          ))}
        </tbody>
