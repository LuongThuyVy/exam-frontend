...
const ExamManagement = () => {
  ...
  const handleShowModal = (type, exam = {}) => {
    ...
    setCurrentExam({
      ...
      SubjectId: exam.subjectId || '', // Use subjectId if available
      GradeId: exam.gradeId || '', // Use gradeId if available
    });
    setShowModal(true);
    setError('');
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const examData = {
        ...
        SubjectId: parseInt(currentExam.SubjectId), // Ensure value is sent
        GradeId: parseInt(currentExam.GradeId), // Ensure value is sent
      };
        ...
        <tbody>
          {exams.map(exam => (
            <tr key={exam.id}>
              ...
              <td>{subjects.find(subject => subject.id === exam.subjectId)?.name || ''}</td>
              <td>{grades.find(grade => grade.id === exam.gradeId)?.name || ''}</td>
              <td>
