...
function QuestionsPage() {
  ...
  const handleFormSubmit = (event) => {
    ...
    const url = editingQuestion ? 
      `http://localhost:8000/api/questions/${editingQuestion.id}` : 
    ...
    axios({ method, url, data: formData })
      ...
      .catch(error => console.error(`Error ${editingQuestion ? 'updating' : 'adding'} question:`, error));
  };
  

  const handleEditClick = (question) => {
    console.log('Editing Question ID:', question.id); // Log question ID
    ...
    setFormData({
      ...
      SubjectId: question.SubjectId || '', // Update based on actual structure
      GradeId: question.GradeId || '' // Update based on actual structure
    });
    setShowModal(true);
  };
  
  const handleDeleteClick = (questionId) => {
    console.log('Deleting Question ID:', questionId); // Log question ID
