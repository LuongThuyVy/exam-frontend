...
const ExamManagement = () => {
  ...
  const handleShowModal = (type, exam = {}) => {
    ...
    setCurrentExam({
      ...
      SubjectGradeId: exam.subjectGrade ? exam.subjectGrade.Id : '',
    });
    setShowModal(true);
    setError('');
  };
  ...
  return (
    <div className="container mt-5">
      ...
      <Table striped bordered hover>
        ...
        <tbody>
          {exams.map(exam => {
            // Find the corresponding subject grade
            const subjectGrade = subjectGrades.find(sg => sg.Id === exam.SubjectGradeId) || {};
            return (
              <tr key={exam.id}>
                <td>{exam.id}</td>
                <td>{exam.name}</td>
                ...
                <td>{subjectGrade.subject ? subjectGrade.subject.Name : ''}</td>
                <td>{subjectGrade.grade ? subjectGrade.grade.Name : ''}</td>
                <td>
            ...
            <Form.Group controlId="formSubjectGrade">
              ...
              >
                ...
                {subjectGrades.map(subjectGrade => (
                  <option key={subjectGrade.Id} value={subjectGrade.Id}>
                    {subjectGrade.subject.Name} - {subjectGrade.grade.Name}
                  </option>
                ))}
              </Form.Control>
            </Form.Group>
