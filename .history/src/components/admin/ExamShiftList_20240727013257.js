...
const ExamShiftList = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('New Shift Data:', newShift); // Debugging statement
  
    try {
      if (editShift) {
        // Update existing shift
        console.log('Updating shift', editShift.Id); // Debugging statement
        const response = await axios.put(`http://localhost:8000/api/exam-shifts/${editShift.Id}`, newShift);
        console.log('Update response:', response.data); // Debugging statement
      } else {
        // Add new shift
        console.log('Adding new shift'); // Debugging statement
        const response = await axios.post('http://localhost:8000/api/exam-shifts', newShift);
        console.log('Add response:', response.data); // Debugging statement
      }
      // Refresh the list
