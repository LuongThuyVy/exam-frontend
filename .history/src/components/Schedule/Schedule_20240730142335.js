...
const ExamShiftCard = () => {
  ...
  const calculateRemainingTime = (startTime) => {
    ...
    if (timeDifference > 0) {
      ...
      return minutes <= 5 ? `${minutes}m remaining` : `${Math.floor(minutes / 60)}h ${minutes % 60}m remaining`;
    }
    ...
    <Container className="mt-4">
      ...
      <Row>
        {examShifts.map(shift => {
          const remainingTime = calculateRemainingTime(shift.examShift.startTime);
          const currentTime = new Date().getTime();
          ...
          const isNotYetAvailable = startTime > currentTime;
          const isNearStart = timeLeft > 0 && timeLeft <= 5 * 60 * 1000; // 5 minutes in milliseconds
                  ...
                  <Row>
                    ...
                    <Col>
                      <Button
                        ...
                        className="w-100"
                        disabled={isExpired || isNotYetAvailable && !isNearStart}
                      >
                        Start Test
                      </Button>
                    </Col>
