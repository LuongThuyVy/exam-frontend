...
const Test = () => {
  ...
  const confirmSubmit = useCallback(async () => {
    try {
      // Get examShiftId and duration from localStorage
      const examShiftId = localStorage.getItem('examShiftId');
      const examShiftId = localStorage.getItem('testId');
      const duration = localStorage.getItem('duration');
      const totalDuration = parseInt(duration, 10) * 60; // Convert duration to seconds
      
      // Check if examShiftId is available
      if (!examShiftId) {
        throw new Error('No examShiftId found');
      }
  
      // Fetch the testId using the examShiftId
      const examShiftResponse = await axios.get(`http://localhost:8000/api/exam-shifts/${examShiftId}`);
      const testId = examShiftResponse.data.testId;
  
      // Check if testId is available
      if (!testId) {
        throw new Error('No testId found for the given examShiftId');
      }
      ...
      const completionTime = totalDuration - remainingTime;
      ...
      // Navigate to the result page or any other page
