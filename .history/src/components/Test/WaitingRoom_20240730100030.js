  ...
  useEffect(() => {
    const fetchExamShifts = async () => {
      try {
        ...
        // Check if warning needs to be set for multiple exam shifts
        if (shifts.length === 2) {
          setWarning('You can only choose one exam shift to test. Please chose');
        } else {
          setWarning(null);
        }
              ...
              <Card.Body>
                ...
                <div className="exam-shift-details">
                  ...
                  {shift.examShift.endTime && (
                    <p className="text-danger text-danger"><b>
                      <i className="bi bi-exclamation-triangle-fill text-danger me-2"></i>Exam shift is ending in
