...
const TestDetails = () => {
    ...
    return (
        <div>
            ...
            >
                {showDetails ? 'Hide Questions and Answers' : 'Show Questions and Answers'}
            </button>

            {/* Conditional Rendering of Questions and Answers */}
            {showDetails && (
                <div>
                    <h2 className='mt-4'>Questions and Answers</h2>
                    {test.answers && test.answers.length > 0 ? (
                        <ul>
                            {test.answers.map((answer, index) => (
                                <li key={answer.Id}>
                                    <p><strong>Question {index + 1}:</strong> {answer.questionAnswer.Content}</p>
                                    <p><strong>Your Answer:</strong> {answer.SelectedOption}</p>
                                    <p><strong>Correct Answer:</strong> {answer.question_answer.CorrectOption}</p>
                                </li>
                            ))}
                        </ul>
                    ) : (
