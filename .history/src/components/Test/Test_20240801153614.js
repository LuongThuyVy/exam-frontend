...
const Test = () => {
  ...
  const confirmSubmit = useCallback(async () => {
    try {
      const examShiftId = localStorage.getItem('    setShowWarningModal(false);
');
      if (!examShiftId) {
        throw new Error('No examShiftId found');
      }
  
      // Fetch the testId using the examShiftId (Example API call)
      const testResponse = await axios.get(`http://localhost:8000/api/exam-shifts/${examId}/test`);
