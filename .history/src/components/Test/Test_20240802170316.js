...
const Test = () => {
  ...
  const confirmSubmit = useCallback(async () => {
    // try {
      ...
      // const duration = localStorage.getItem('duration');
      // const totalDuration = parseInt(duration, 10) * 60; // Thời gian tổng cộng
      ...
      //   time: completionTime
      const testId = 45; // Ví dụ TestId
const formattedAnswers = [{
  QuestionAnswerId: 13,
  SelectedOption: 'A'
}]};
const completionTime = 120; // Ví dụ thời gian hoàn thành
...
const response = await axios.post('http://localhost:8000/api/tests/submit', {
  ...
  time: completionTime
      ...
      console.error('Error submitting answers:', error.response.data || error.message || error);
      alert('There was an error submitting your answers. Please check the console for details.');
    }
