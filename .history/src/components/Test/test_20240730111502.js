...
const Test = () => {
  ...
  const renderQuestionStatus = () => {
    ...
    const rows = [];
    const numColumns = 4; // Number of columns in the table
    ...
    for (let i = 0; i < totalQuestions; i += numColumns) {
      ...
      for (let j = 0; j < numColumns; j++) {
        ...
        const cellClass = isAnswered ? 'bg-success text-light' : 'bg-danger text-light';

        columns.push(
          <td key={j} className={`p-2 text-center ${cellClass}`} style={{ cursor: 'pointer' }}>
            {questionIndex + 1}
          </td>
        );
      }
