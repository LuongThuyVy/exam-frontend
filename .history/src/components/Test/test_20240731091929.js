...
const Test = () => {
  ...
  const [showConfirmModal, setShowConfirmModal] = useState(false); // To control the confirmation modal
  const [completionTime, setCompletionTime] = useState(null); // To store the time when the user completes the test
  ...
  const confirmSubmit = async () => {
    try {
      ...
      const completionTime = new Date(Date.now() + remainingTime * 1000).toISOString(); // Convert remaining time to ISO string
      localStorage.setItem('completionTime', completionTime);

      // Include completionTime in the payload
      ...
      <Modal show={showConfirmModal} onHide={() => setShowConfirmModal(false)}>
        ...
        <Modal.Footer>
          ...
          <Button
            variant="primary"
            onClick={() => {
              setCompletionTime(remainingTime);
              confirmSubmit();
