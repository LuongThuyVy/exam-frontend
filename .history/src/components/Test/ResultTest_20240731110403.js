                    ...
                    {test.answers && test.answers.length > 0 ? (
                        <div className='row'>
                            {test.answers.map((answer, index) => (
                                <div className='col-md-6 mb-4' key={answer.Id}>
                                    <div className='card'>
                                        <div className='card-body'>
                                            <p className='card-text'><strong>Question {index + 1}:</strong> {answer.question_answer.Content}</p>
                                            <p className='card-text'>Your Answer:</strong> {answer.SelectedOption}. {getOptionContent(answer.question_answer, answer.SelectedOption)}</p>
                                            <p className='card-text text-success'><strong>Correct Answer:</strong> {answer.question_answer.CorrectOption}. {getOptionContent(answer.question_answer, answer.question_answer.CorrectOption)}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
