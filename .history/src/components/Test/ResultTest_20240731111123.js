                    ...
                    {test.answers && test.answers.length > 0 ? (
                        <div className='d-flex flex-wrap justify-content-center'>
                            {test.answers.map((answer, index) => {
                                ...
                                return (
                                    <div className='card' style={{ width: '18rem', margin: '10px' }} key={answer.Id}>
                                        <div className='card-body'>
                                            <p className='card-text'><strong>Question {index + 1}:</strong> {answer.question_answer.Content}</p>
                                            <p className={`card-text ${!isCorrect ? 'text-danger' : ''}`}><strong>Your Answer:</strong> {answer.SelectedOption}. {getOptionContent(answer.question_answer, answer.SelectedOption)}</p>
                                            {!isCorrect && (
