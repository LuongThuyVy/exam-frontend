...
const ExamShiftCard = () => {
  ...
  const handleStartTest = async (shiftId) => {
    try {
        ...
        if (response.data.message === 'Test started successfully') {
            setModalMessage('Your exam has started. Redirecting to the test...');
            setTimeout(() => {
                navigate('/test');
            }, 2000); // Delay before redirection for better user experience
        } else {
            setModalMessage('The test cannot be started yet. Please check back later.');
        }
    } catch (err) {
  ...
  return (
    <Container className="mt-4">
      ...
      <p>All your exams has started. Start now!</p>
