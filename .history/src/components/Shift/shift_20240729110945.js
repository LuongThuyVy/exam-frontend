...
const ExamShiftCard = () => {
  ...
  const handleSignIn = async (id, formData) => {
    try {
      ...
      console.log('Examinee ID from local storage:', examineeId);
      console.log('Exam Shift ID passed to function:', id);
      ...
      const signInResponse = await axios.post('http://localhost:8000/tests', {
        ExamineeId: examineeId,
        ExamShiftId: id
      });
  
      if (signInResponse.status === 201) {
        console.log('Sign-in successful, response:', signInResponse.data);
  
        // Perform the registration request
        try {
          const registrationResponse = await axios.post('http://127.0.0.1:8000/api/register', formData);
  
          if (registrationResponse.status === 201) {
            console.log('Registration successful, response:', registrationResponse.data);
