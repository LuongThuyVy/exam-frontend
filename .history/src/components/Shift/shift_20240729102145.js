      ...
      <Row>
        {examShifts.map(shift => (
          <Col md={4} key={shift.id} className="mb-4">
            <Card>
              <Card.Header>{shift.subjectgrade && shift.subjectgrade.Name ? shift.subjectgrade.Name : 'Unknown Subject'}</Card.Header>
              <Card.Body>
                <Card.Title>
                  Grade: {shift.subjectgrade && shift.subjectgrade.grade && shift.subjectgrade.grade.Name ? shift.subjectgrade.grade.Name : 'Unknown Grade'}
                </Card.Title>
                <Card.Text>
                  <strong>Date:</strong> {shift.startTime ? new Date(shift.startTime).toLocaleDateString() : 'Unknown Date'}<br />
