  ...
  useEffect(() => {
    const fetchExamShifts = async () => {
      try {
        ...
        const accountId = userData ? JSON.parse(userData).id : null;

        // Determine API endpoint based on login status
        const apiUrl = accountId ? 
          `http://localhost:8000/api/exam-shifts/user/${accountId}` :
        ...
        setIsLoggedIn(!!accountId); // Set login status based on presence of accountId
      } catch (err) {
  ...
  const handleSignIn = async (id) => {
    try {
      ...
      if (!userData) {
        ...
        setError('User data not found. Redirecting to login.');
      ...
      if (!examineeId) {
        ...
        setError('Examinee ID is missing. Redirecting to login.');
        navigate('/login'); // Redirect to login page if Examinee ID is missing
        return;
      }
              ...
              <Card.Footer>
                <Row>
                  ...
                  <Col>
                    <Button
                      variant="info"
                      onClick={() => isLoggedIn ? handleSignIn(shift.id) : navigate('/login')}
                      className="w-100"
                    >
                      {isLoggedIn ? 'Sign In' : 'Login to Sign In'}
                    </Button>
                  </Col>
                </Row>
              </Card.Footer>
