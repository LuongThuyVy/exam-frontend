...
const TestList = () => {
    ...
    useEffect(() => {
        ...
        const accountId = JSON.parse(localStorage.getItem('user'))?.id; // Thay đổi giá trị mặc định nếu cần
                ...
                <tbody>
                    {tests.map(test => (
                        <tr key={test.testId}>
                            ...
                            <td>{test.examShift.score !== null ? test.examShift.score : 'N/A'}</td>
                            <td>{test.examShift.time ? new Date(test.examShift.time).toLocaleString() : 'N/A'}</td>
                        </tr>
                    ))}
                </tbody>
            </Table>
