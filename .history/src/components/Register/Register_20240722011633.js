...
function Register() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    password: '',
    // policyAccepted: false, // Đảm bảo khởi tạo giá trị checkbox
  });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('Data sent to register API:', formData);  // Log dữ liệu gửi đi

    axios.post('http://127.0.0.1:8000/api/register', formData)
      .then(response => {
        console.log('Response from register API:', response.data);  // Log dữ liệu nhận về
      ...
      .catch(error => {
        if (error.response && error.response.data && error.response.data.errors) {
          ...
          console.log('Error response from register API:', error.response.data.errors);  // Log lỗi nhận về
          toast.error('An error occurred during registration');
        } else {
          setErrors({ general: 'An error occurred during registration.' });
          console.log('General error:', error);  // Log lỗi chung
          toast.error('An error occurred during registration');
        }
      });
  };
          ...
          <>
            <Form.Group controlId="formEmail" className="mt-3">
              ...
              <Form.Control
                type="email"
                name="email"
                value={formData.email || ''} // Đảm bảo giá trị không undefined
                onChange={handleChange}
            ...
            <Form.Group controlId="formPassword" className="mt-3">
              ...
              <Form.Control
                type="password"
                name="password"
                value={formData.password || ''} // Đảm bảo giá trị không undefined
                onChange={handleChange}
            ...
            <Form.Group controlId="formPolicyAccepted" className="mt-3">
              <Form.Check
                type="checkbox"
                name="policyAccepted"
                checked={formData.policyAccepted || false} // Đảm bảo giá trị không undefined
                onChange={handleChange}
