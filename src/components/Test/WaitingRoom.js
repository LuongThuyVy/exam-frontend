...
const WaitingRoom = () => {
  ...
  useEffect(() => {
    const fetchExamShifts = async () => {
      try {
        ...
        // Fetch exam shifts
        const response = await axios.get(`http://localhost:8000/api/waiting-room/${accountId}`);
  ...
  const handleConfirmStart = () => {
    if (selectedExamShift) {
      const examId = selectedExamShift.testId; // Get testId
      const examShiftId = selectedExamShift.examShift.id; // Get examShiftId
      ...
      // Store both testId and examShiftId in local storage
      localStorage.setItem('testId', examId);
      localStorage.setItem('examShiftId', examShiftId);
